...
function TtsButton({ text }: { text: string }) {
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
    >
      <Feather name={playing ? "volume-x" : "volume-2"} size={12} color={playing ? colors.destructive : colors.mutedForeground} />
  ...
  return (
    <View style={[s.bubbleWrap, s.bubbleWrapAssistant]}>
      ...
      <View style={[s.bubble, s.assistantBubble]}>
        ...
        {msg.content ? <TtsButton text={msg.content} /> : null}
      </View>
    </View>
  );
}
  ...
  return (
    <SafeAreaView style={s.container} edges={["top"]}>
      ...
      <Modal visible={showTranslateModal} transparent animationType="slide" onRequestClose={() => setShowTranslateModal(false)}>
        <Pressable style={s.gateOverlay} onPress={() => setShowTranslateModal(false)}>
          <Pressable style={[s.gateSheet, { backgroundColor: colors.card }]} onPress={() => {}}>
            ...
            <Text style={[s.gateTitle, { color: colors.foreground }]}>Translate with AI</Text>
            <Text style={[s.gateDesc, { color: colors.mutedForeground }]}>
              Translate text or the selected conversation into your preferred language.
            </Text>
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, justifyContent: "center", paddingVertical: 8 }}>
              {["English", "Spanish", "French", "German", "Chinese", "Japanese", "Arabic", "Portuguese", "Russian", "Hindi"].map((lang) => (
                <TouchableOpacity
                  key={lang}
                  style={[
                    s.providerPill,
                    { backgroundColor: translateTarget === lang ? colors.primary : colors.muted },
                  ]}
                  onPress={() => setTranslateTarget(lang)}
                  activeOpacity={0.7}
                >
                  <Text style={[s.providerPillText, translateTarget === lang && s.providerPillTextActive]}>
                    {lang}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                const text = input || (messages.length > 0 ? messages[messages.length - 1].content : "");
                if (text) {
                  setInput(`Translate the following text into ${translateTarget}. Detect the source language automatically:\n\n${text}`);
                  setShowTranslateModal(false);
                }
              }}
              activeOpacity={0.85}
            >
              <Text style={s.gateUpgradeBtnText}>Translate</Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={[s.gateDismissBtnText, { color: colors.mutedForeground }]}>Cancel</Text>
            </TouchableOpacity>
          </Pressable>
        </Pressable>
      </Modal>
